...
export const updateAccessToken = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const session = await redis.get(decoder.id as string);
      if (!session) {
        return next(new ErrorHandler("Please Login to access this session", 400));
      }
...
export const updateFilePath = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
